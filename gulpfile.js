...
gulp.task('twig', function (done) {
    return gulp.src('src/_jump-to-nav.twig')
        .pipe(twig())
...
gulp.task('js', function (done) {
    ...
    // Save css string
    let cssFileContent = fs.readFileSync('dist/temp/jump-to-nav.css', 'utf8');
    
    // Save html string
    let modalFileContent = fs.readFileSync('dist/temp/_jump-to-nav.html', 'utf8');
        
    return gulp.src([`${roots.src}/js/jump-to-nav.js`], { sourcemaps: true })
        .pipe(include())
        .pipe(concat('jump-to-nav.js'))
        
        // Inject css string into js
        .pipe(inject.replace('//import jump-to-nav.css', cssFileContent.trim() ))
        
        // Inject html string into js
        .pipe(inject.replace('//import _jump-to-nav.html', modalFileContent.trim() ))
        
        // Remove scss sourcemaps linkage from string
        .pipe(gulpReplace('/*# sourceMappingURL=jump-to-nav.css.map */', ''))
